  ...
  showResults(question: Quizz["questions"][number]) {
    ...
    const sortedPlayers = this.players
      .map((player) => {
        ...
        const points = answered ? Math.round(playerAnswer.points) : 0

        player.points += points

        return { ...player, lastAnswered: answered, lastPoints: points }
      })
    ...
    sortedPlayers.forEach((player, index) => {
      ...
      this.sendStatus(player.id, STATUS.SHOW_RESULT, {
        answered: player.lastAnswered,
        message: player.lastAnswered ? "Answer registered!" : "No answer",
